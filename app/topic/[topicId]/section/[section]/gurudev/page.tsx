...
import { GurudevView } from "./gurudev-view";

interface Props {
  params: Promise<{ topicId: string; section: string }>;
}

export default async function GurudevPage({ params }: Props) {
  ...
  if (!section || !section.gurudevInsight) notFound();

  return <GurudevView topicId={topicId} section={section} />;
}
